...
TODO: convert quaternions to (w, x, y, z) for consistency with Nvidia Isaac
"""
...
def w_quat(quart):
    """
    Converts quaternion from x,y,z,w to w,x,y,z.
    ...
    if quart is None:
        return None
    return np.array([quart[3], quart[0], quart[1], quart[2]])
...
def x_quat(quart):
    ...
    if quart is None:
        return None
    return np.array([quart[1], quart[2], quart[3], quart[0]])
...
def transform_by_matrix(matrixA, matrixB):
    """
    Transform matrix A by matrix B.

    Args:
        matrixA (np.array): 2d-array representing a matrix
        matrixB (np.array): 2d-array representing a matrix

    Returns:
        np.array: 2d-array representing a tranformed matrix
    """
    return matrixA @ matrixB

def create_transformation_matrix(pos, ori):
    """
    Helper function to create a transformation matrix from a position and orientation.

    Args:
        pos (np.array): 1d-array representing the position
        ori (np.array): 2d-array representing the orientation
