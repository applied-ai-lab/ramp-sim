...
from core.utils.control_utils import *
...
class OperationalSpaceController(object):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.input_max = nums2array(input_max, self.control_dim)
        self.input_min = nums2array(input_min, self.control_dim)
        self.output_max = nums2array(output_max, self.control_dim)
        self.output_min = nums2array(output_min, self.control_dim)

        # kp kd
        self.kp = nums2array(kp, 6)
        self.kd = 2 * np.sqrt(self.kp) * damping_ratio

        # kp and kd limits
        self.kp_min = nums2array(kp_limits[0], 6)
        self.kp_max = nums2array(kp_limits[1], 6)
        self.damping_ratio_min = nums2array(damping_ratio_limits[0], 6)
        self.damping_ratio_max = nums2array(damping_ratio_limits[1], 6)
    ...
    def set_goal(self, action, ee_pose, set_pos=None, set_ori=None):
        """
        ...
        Args:
            ...
            ee_pose (Iterable): Current EE pose
        ...
        if sum(bools) > 0.0 or set_ori is not None:
            self.goal_ori = set_goal_orientation(
                quat2axisangle(x_quat(scaled_delta[3:7])), quat2mat(x_quat(ee_pose[3:7])), orientation_limit=self.orientation_limits, set_ori=set_ori
            )
        self.goal_pos = set_goal_position(
            scaled_delta[:3], ee_pose[:3], position_limit=self.position_limits, set_pos=set_pos
        )

        if self.interpolator_pos is not None:
            self.interpolator_pos.set_goal(self.goal_pos,ee_pose[:3])

        if self.interpolator_ori is not None:
            self.ori_ref = np.array(quat2mat(x_quat(ee_pose[3:7])))  # reference is the current orientation at start
    ...
    def run_controller(self, jacobian, position_jacobian, orientation_jacobian, ee_pose, ee_vel, joint_pos, joint_vel, mass_matrix, gravity):
        """
        ...
        position_error = desired_pos - ee_pose[:3]
        vel_pos_error = -ee_vel[:3]
        ...
        lambda_full, lambda_pos, lambda_ori, nullspace_matrix = opspace_matrices(
            mass_matrix, jacobian, position_jacobian, orientation_jacobian
        )
        ...
        self.torques = np.squeeze(np.dot(jacobian.T, decoupled_wrench)) + gravity
    ...
    def update_initial_joints(self, initial_joints):
        ...
        self.initial_joint = initial_joints
